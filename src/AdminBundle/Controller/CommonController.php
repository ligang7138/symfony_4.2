...
abstract class CommonController extends AbstractController
{
	...
	protected function show($view = 'index/index', $data = []){
		...
		if ($curUser) {
			...
			if($curUser->getAType() == 0){
				...
				$partner = $em->getRepository('App\AdminBundle:YsPartners')->findOneBy(['adminId' => $curUser->getAid()]);
		...
		if($request->headers->contains('accept','application/json')){
	        ...
	        $header = $this->container->get('twig')->render("@App\AdminBundle:{$theme_type}/theme/$dir:header.html.twig", $data);
	        $body = $this->container->get('twig')->render("@App\AdminBundle:{$theme_type}/" . str_replace('/', ':', $view) . '.html.twig', $data);
	        $footer = $this->container->get('twig')->render("App\AdminBundle:{$theme_type}/theme/$dir:footer.html.twig", $data);
    ...
    {
	    if(strpos($view,'App\AdminBundle') !== false){
            $theme_type = $this->getThemeType();
            $view = str_replace('App\AdminBundle:',"App\AdminBundle:{$theme_type}/",$view);
        }
