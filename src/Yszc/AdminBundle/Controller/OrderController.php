...
//	    $conditions['search_key'] = $request->get('search_key','');
	...
	*/
	public function orderModify(Request $request){
		...
		$order = $this->getDoctrine()->getRepository('AdminBundle:YsOrder')->find($order_id);
	...
	public function orderDetail(Request $request){
		...
		$goods = $this->getOrderService()->getOrderGoodsList($order_id);
		$logList = $this->getDoctrine()->getRepository('AdminBundle:YsLogs')->findBy(['orderId'=>$order_id]);
	...
	public function confirmDeliverGoods(Request $request){
		...
		if(in_array($this->getUser()->getAType(),[1,2])){
			$order = $this->getDoctrine()->getRepository('AdminBundle:YsOrder')->findOneBy(['orderId'=>$order_id]);
		}else{
			$order = $this->getDoctrine()->getRepository('AdminBundle:YsOrder')->findOneBy(['orderId'=>$order_id,'partnerId'=>intval($request->getSession()->get('partner_id'))]);
		}
		if($order){
			$em = $this->getDoctrine()->getManager();
