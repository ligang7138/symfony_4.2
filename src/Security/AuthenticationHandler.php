...
//        $admin = $this->service->getRepository('AdminBundle:AdminUsers');
	...
	protected function pullMsg(AdminUsers $user){
		...
		$aid = $user->getAId();
		$last_pull_data = $conn->fetchAssoc("select max(create_time) as last_time from qy_user_message where u_code = {$aid} order by create_time desc");
		...
		$msgIdArr = $conn->fetchAll("select msg_id from qy_user_message where u_code = {$aid}");
		$msgIdStr = implode(array_column($msgIdArr,'msg_id'),',');
		if(empty($msgIdStr)){
			$sql = "select * from qy_message where (admin_id = {$aid} or (msg_sys_type in (1,3) and msg_send_type = 2 and msg_send_status = 2)) and msg_add_time >= '{$last_time}' ";

		}else{
			$sql = "select * from qy_message where (admin_id = {$aid} or (msg_sys_type in (1,3) and msg_send_type = 2 and msg_send_status = 2)) and msg_add_time >= '{$last_time}' and msg_id not in($msgIdStr)";
		}
		$data = $conn->fetchAll($sql);
		if(!empty($data)){
			$insertSql = 'insert into qy_user_message(u_code,msg_id,read_status,create_time) values ';
			foreach ($data as $v){
